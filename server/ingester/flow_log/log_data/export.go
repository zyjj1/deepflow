...
func (l4 *L4FlowLog) EncodeTo(protocol config.ExportProtocol, utags *utag.UniversalTagsManager, cfg *config.ExporterCfg) (interface{}, error) {
	...
	case config.PROTOCOL_KAFKA:
		...
		return common.EncodeToJson(l4, int(l4.DataSource()), cfg, tags0, tags1), nil
	default:
		return nil, fmt.Errorf("unsupport export to %s", protocol)
	}
}
...
func (l7 *L4FlowLog) GetFieldValueByOffsetAndKind(offset uintptr, kind reflect.Kind, fieldName string) interface{} {
	return utils.GetValueByOffsetAndKind(uintptr(unsafe.Pointer(l7)), offset, kind, fieldName)
}
...
func (l7 *L7FlowLog) EncodeTo(protocol config.ExportProtocol, utags *utag.UniversalTagsManager, cfg *config.ExporterCfg) (interface{}, error) {
	...
	case config.PROTOCOL_KAFKA:
		...
		return common.EncodeToJson(l7, int(l7.DataSource()), cfg, tags0, tags1), nil
	default:
		return nil, fmt.Errorf("unsupport export to %s", protocol)
	}
}
...
func (l7 *L7FlowLog) GetFieldValueByOffsetAndKind(offset uintptr, kind reflect.Kind, fieldName string) interface{} {
	return utils.GetValueByOffsetAndKind(uintptr(unsafe.Pointer(l7)), offset, kind, fieldName)
}
