...
func (e *ExportDocumentFlow) GetFieldValueByOffsetAndKind(offset uintptr, kind reflect.Kind, fieldName string) interface{} {
	return utils.GetValueByOffsetAndKind(uintptr(unsafe.Pointer(e)), offset, kind, fieldName)
}
...
func (e *ExportDocumentApp) GetFieldValueByOffsetAndKind(offset uintptr, kind reflect.Kind, fieldName string) interface{} {
	return utils.GetValueByOffsetAndKind(uintptr(unsafe.Pointer(e)), offset, kind, fieldName)
}
...
func (e *ExportDocumentUsage) GetFieldValueByOffsetAndKind(offset uintptr, kind reflect.Kind, fieldName string) interface{} {
	return utils.GetValueByOffsetAndKind(uintptr(unsafe.Pointer(e)), offset, kind, fieldName)
}
...
func EncodeTo(e app.Document, protocol config.ExportProtocol, utags *utag.UniversalTagsManager, cfg *config.ExporterCfg) (interface{}, error) {
	...
	case config.PROTOCOL_KAFKA:
		...
		return exportercommon.EncodeToJson(e, int(e.DataSource()), cfg, tags0, tags1), nil
...
func getPrometheusLabels(e app.Document, uTags0, uTags1 *utag.UniversalTags, cfg *config.ExporterCfg) []prompb.Label {
	...
	labels = append(labels, prompb.Label{
		Name:  "datasource",
		Value: config.DataSourceID(e.DataSource()).String(),
	})
	var valueStr string
	...
	for _, structTags := range cfg.ExportFieldStructTags[dataSourceId] {
		...
		value := e.GetFieldValueByOffsetAndKind(structTags.Offset, structTags.DataType, structTags.FieldName)
		if utils.IsNil(value) {
			log.Info("is nil ", structTags.FieldName)
			continue
		}
		...
		if structTags.Omitempty && valueStr == "" {
			continue
		}

		if structTags.TranslateFile != "" {
			valueStr = structTags.TranslateStringMap[valueStr]
		}
...
func EncodeToPrometheus(e app.Document, utags *utag.UniversalTagsManager, cfg *config.ExporterCfg) (interface{}, error) {
	...
	for _, structTags := range cfg.ExportFieldStructTags[dataSourceId] {
		if structTags.CategoryBit&config.METRICS == 0 && structTags.Name != "" {
			continue
		}
		isFloat64 := false
		value := e.GetFieldValueByOffsetAndKind(structTags.Offset, structTags.DataType, structTags.FieldName)
		...
		if structTags.Omitempty && valueFloat64 == 0 {
			continue
		}
