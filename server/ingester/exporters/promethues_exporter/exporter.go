...
func (e *PrometheusExporter) queueProcess(queueID int) {
	...
	doReq := func() {
		...
		if err := e.sendRequest(batchs); err != nil {
	...
	for e.running {
		...
		for _, item := range items[:n] {
			...
			timeSeries, ok := ts.([]prompb.TimeSeries)
			if !ok {
				continue
			}
...
func (e *PrometheusExporter) sendRequest(batchs []prompb.TimeSeries) error {
	...
	req, err := http.NewRequestWithContext(e.ctx, "POST", e.config.Endpoints[0], bytes.NewReader(compressedData))
	if err != nil {
		return err
	}
