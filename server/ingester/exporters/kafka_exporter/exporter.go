...
func (e *KafkaExporter) queueProcess(queueID int) {
	...
	for e.running {
		...
		for _, item := range items[:n] {
			...
			exportItem.Release()
			if err != nil && e.counter.DropCounter == 0 {
			...
			batch = append(batch,
				&sarama.ProducerMessage{
					...
					Key:       nil,
					Value:     sarama.ByteEncoder(utils.Slice(jsonStr)), // 设置消息值为压缩前的 JSON 数据
					Timestamp: time.Now(),
				},
			)
...
func (e *KafkaExporter) exportBatch(queueID int, batch []*sarama.ProducerMessage) {
	...
	if utils.IsNil(e.producers[queueID]) {
		...
		if err != nil && e.counter.DropCounter == 0 {
			log.Warningf("exporter %d new kafka producer failed. err: %s", e.index, err)
