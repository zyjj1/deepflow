...
func (a *Aliyun) getNatGateways(region model.Region) (
	...
	for _, r := range response {
		for i := range r.Get("NatGateway").MustArray() {
			...
			natGatewayLcuuid := a.generateLCUUID(natGatewayId)
			...
			// 接口
			vinterfaceLcuuid := a.generateLCUUID(natGatewayLcuuid)
			...
			for _, ip := range floatingIPs {
				ipLcuuid := a.generateLCUUID(vinterfaceLcuuid + ip)
				retIP := model.IP{
					Lcuuid:           ipLcuuid,
		...
		for _, s := range response {
			for j := range s.Get("SnatTableEntry").MustArray() {
				...
				retNATRule := model.NATRule{
					Lcuuid:           a.generateLCUUID(snatId),
					NATGatewayLcuuid: a.generateLCUUID(natGatewayId),
					Type:             "SNAT",
					Protocol:         "ALL",
