...
func NewKubernetesGatherTask(
	orgID int, ctx context.Context, domain *mysql.Domain, subDomain *mysql.SubDomain, cfg config.CloudConfig, isSubDomain bool) *KubernetesGatherTask {
	kubernetesGather := kubernetes_gather.NewKubernetesGather(orgID, domain, subDomain, cfg, isSubDomain)
